...
@Component({
  selector: 'app-cutbanacerouti',
  templateUrl: './cutbanacerouti.component.html',
  styleUrls: ['./cutbanacerouti.component.css'],
})
export class CutbanaceroutiComponent implements OnInit {
  oldbal: any;
      ...
      this.httpservice.updateCustomerData(this.customerData, id).subscribe(
        (res) => {
          ...
          setTimeout(() => {
            ...
            this.routes.navigate(['/removebalance']);
          }, 1000);
        },
        (error) => {
          console.error(error);
